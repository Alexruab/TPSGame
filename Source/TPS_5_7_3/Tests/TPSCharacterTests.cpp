...
#include "TPS_5_7_3/Tests/TPSCharacterTests.h"
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FAutoHealChouldRestoreHealth, "TPSGame.Character.AutoHealChouldRestoreHealth", 
	EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::ProductFilter | EAutomationTestFlags::HighPriority);

namespace
{
	constexpr const char* CharacterTestBPName = "Blueprint'/Game/Tests/BP_TestThirdPersonCharacter.BP_TestThirdPersonCharacter'";
}
...
bool FHealthMightBeChangeWithDamage::RunTest(const FString& Parameters)
{
	...
	auto Character = CreateBlueprintDeferred<ATPS_5_7_3Character>(World, CharacterTestBPName, InitialTransform);
	if (!TestNotNull("Inventory component exist",Character)) return false;
...
{           
	UE_LOG(LogTPSCharacterTests, Warning, TEXT("Log 1"));
	ADD_LATENT_AUTOMATION_COMMAND(FTPSLatentLogCommand("Latent log 1"));
	UE_LOG(LogTPSCharacterTests, Warning, TEXT("Log 2"));
	ADD_LATENT_AUTOMATION_COMMAND(FTPSLatentLogCommand("Latent log 2"));
	UE_LOG(LogTPSCharacterTests, Warning, TEXT("Log 3"));
...
DEFINE_LATENT_AUTOMATION_COMMAND_TWO_PARAMETER(FCharacterDestroyedLatendCommand, ATPS_5_7_3Character*, Character, float, LifeSpan);

bool FCharacterDestroyedLatendCommand::Update()
{
...
bool FCharacterCanBeKilled::RunTest(const FString& Parameters)
{
	...
	//ADD_LATENT_AUTOMATION_COMMAND(FCharacterDestroyedLatendCommand(Character, HealthData.LifeSpan));
...
DEFINE_LATENT_AUTOMATION_COMMAND_TWO_PARAMETER(FAutoHealCheckLatentCommand, ATPS_5_7_3Character*, Character, float, HealingDuration);
...
bool FAutoHealChouldRestoreHealth::RunTest(const FString& Parameters)
{
	const auto Level = LevelScope("/Game/Tests/EmptyTestLevel");
	...
	const float HealingDuration = (DamageAmount / HealthData.HealModifier) * HealthData.HealRate;
